...
Other technical issues must be taken into account when performing such association, since commonly used measures of LD (such as $D'$) are known to give rise to large linkage when sampling minor allele frequencies (MAF) near 0 \cite{koch2013long}.
An alternative measure is the probability that a large value of the disequilibrium $D$ is observed if there is no association.
This can be further refined by conditioning on the sampled allele frequencies (at the two loci), which converges asymptotically to a Fisher's exact test \cite{koch2013long}.
