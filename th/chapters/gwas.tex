...
%Figure \ref{fig:q_q2_compare} shows that the number of pairs for which $R$ deviates significantly from 1 is quite large, arguing that interacting sites have co-evolutionary rates that differ from the bulk of non-interacting sites.
...
In order to asses disease association power we performed extensive simulations, as it is often the case in this kind of analysis some simplifying but realistic assumptions were required to make simulations computationally tractable. 
We assumed that: 
...
iv) that cofactors influencing disease such as population admixture, age and sex were perfectly reduced by the model, meaning that no residual effects remains after correction;
...
As a results form these simulations we observe, as expected, that very large sample sizes are expected to be required to finding epistatic effects on rare variant loci (i.e. allele frequencies below $1\%$).
...
\fig{gwas_Qhat_vs_PAM1}{gwas_Qhat_vs_PAM1}{14cm}{Comparison (relative error) between PAM1 and $\hat{P}(t)$ estimated from $\hat{Q}$
}{Comparison (relative error) between PAM1 and $\hat{P}(t)$ estimated from $\hat{Q}$}

\fig{gwas_Q2}{gwas_Q2}{14cm}{$\hat{Q}_2$ matrix structure}{$\hat{Q}_2$ matrix structure}

\fig{gwas_genegene}{gwas_genegene}{14cm}{Distribution of $\ell_C(k)$ for interacting genes (red) and non-interacting genes (green) showing a small but statistically significant difference.}{Distribution of $\ell_C(k)$ for interacting genes and non-interacting genes}
...
|Significance (CLNSIG)       Count    LL_mean     LL_median    Len_mean   Len_median |
|Benign (2)                   272      34.1         26.2         3925        1172    |
|Likely benign (3)            258      31.5         25.5        11619        4575    | 
|Likely pathogenic (4)        562      17.5         12.0         2132         838    | 
|Pathogenic (5)              4206      16.9         11.7         1057         688    |
|Drug response (6)             18      32.6         22.1          729         492    |
|Other (255)                   10      20.0         11.3         4088         767    |
Table S4A: ClinVar categories have different distributions. Columns 3 and 4 show the mean and median LL(MSA) values calculated for each variant using the best (highest) within protein LL(MSA). Columns 5 and 6 show the mean and median protein length.


| Project      | Mean LL(MSA) | Median LL(MSA) |
| 1000 Genomes |         23.2 |          14.8  |
| HGMD         |         19.8 |          12.1  |
| ClinVar      |         18.6 |          11.9  |
Table S4B: Overall distributions for 1000Genomes, HGMD and ClinVar. Columns 2 and 2 show the mean and median LL(MSA) values calculated for each variant using the best (highest) within protein LL(MSA). Values are calculated using a random sub-sample of all variants in each project.
