...
In this chapter, we propose a novel method to take into account epistatic models into genome wide association studies (GWAS). First we develop a model for epistasis based on a Markov evolutionary theory. Then we fit model parameters using protein structural data an genome wide multiple sequence alignments. Finally, we perform genome wide association using pairs of variants  from a sequencing study by incorporating the epistatic model as priors in a Bayesian Framework.
