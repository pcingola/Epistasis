...
public class ParametersForChainCompletionLayout implements LayoutManager {
    ...
    public void layoutContainer(Container parent) {
        ...
        if (c.isVisible()) {c.setBounds(insets.left+8,insets.top+56,168,24);}
        c = parent.getComponent(3);
        if (c.isVisible()) {c.setBounds(insets.left+8,insets.top+32,168,24);}
        c = parent.getComponent(4);
        if (c.isVisible()) {c.setBounds(insets.left+184,insets.top+56,72,24);}
        c = parent.getComponent(5);
        if (c.isVisible()) {c.setBounds(insets.left+184,insets.top+32,72,24);}
        c = parent.getComponent(6);
        if (c.isVisible()) {c.setBounds(insets.left+136,insets.top+88,72,24);}
    }
}
