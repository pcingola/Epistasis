...
import java.util.*;
import meshi.util.filters.*;import meshi.util.*;
 ...
 *  
 **/
public class DistanceMatrix extends MeshiProgram implements Updateable {
    ...
    public static final double DEFAULT_EDGE(double rMax, double buffer){return rMax+2.0*buffer/3.0;}
    ...
    public DistanceMatrix(MolecularSystem molecularSystem, double rMax, double buffer, double edge,
                         int bondedListDepth) {
	...
	for (int i = 0; i< size; i++) {
	    ...
	    else matrix[i] = new MatrixRow(atom, matrix);
	}
	/**
...
//    public DistanceList newNonBondedList() {return newNonBondedList;}
	...
	try {
	    if ((atom1Number <0) || 
		(atom2Number <0) || 
		(atom1Number >= matrix.length) || 
		(atom2Number >= matrix.length) || 
		(matrix[atom1Number] == null) || 
		(matrix[atom2Number] == null)) return null;
	} catch (RuntimeException ex) {
    ...
    public double radius() { return Utils.radius(molecularSystem);}
    ...
    public static DistanceList getBondedList(MolecularSystem molecularSystem, int depth, MatrixRow[] matrix ) {
	AtomList bonded;
	Atom atom;
	...
	for (int iatom = 0; iatom < size; iatom++) {
		...
		for (Atom bondedAtom:bonded)
		    if (bondedAtom.active()) tempList.add(new AtomPair(atom, bondedAtom));
	    }
	}
	tempList.sort();
	...
	Distance distance;
	out = new DistanceList(size);
	for (AtomPair atomPair:tempList){
	    if ((atomPair.atom1().active()) &
		(atomPair.atom2().active()) &
		(atomPair.atom1().number() < matrix.length) &
		(atomPair.atom2().number() < matrix.length)) {
		...
		if ((matrix[atomPair.largeNumber()] != null) && ( matrix[atomPair.smallNumber()] != null)) {
