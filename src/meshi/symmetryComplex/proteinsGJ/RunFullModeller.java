...
public class RunFullModeller {
    public static final String NAME = "RunFullGJModeller";

    public static void main(String[] args) throws IOException {
       CommandList  commands;

        commands = Utils.init(args,5, Integer.parseInt(args[4]),  // args[0] instead args[2]
                "Usage: java -XmxNNNm "+NAME+"  <commands file name><pdb file for TM helices /or fullsize backBone template for GJ> <sequence><output(path/fileNameMask><seed>\n\n"+
                "NNN is the size of the expected memory requirement in MegaBytes.");
        int seed = Integer.parseInt(args[4]);
        System.out.print(NAME+"\nSeed =\t"+seed+"\n");

        String pdbTemplate = args[1],
            sequence = args[2],
            outputFileMask = args[3];

        BoundariesMap boundsMap = new BoundariesMap(commands); //set static data from file

        FullModeller fullModeller = new FullModeller (
                    pdbTemplate,
                    sequence,
                    commands,
                    outputFileMask+seed
